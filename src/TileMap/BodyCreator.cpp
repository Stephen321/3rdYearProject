...
{
	assert(object.getPolyPointCount() > 1u);
	...
	MapObject::MapObjectType mapObjectType = object.getObjectType();
	sf::Uint16 pointCount = object.getPolyPointCount();

	if (mapObjectType == MapObject::MapObjectType::Polyline || pointCount < 3)
	{
		bodyDef.position.Set(0.f, 0.f);
		bodyDef.position = SfToBoxVec(object.getPosition());
		body = world.CreateBody(&bodyDef);

		const sf::VertexArray& shape = object.getPolyPoints();
		b2FixtureDef f;
		f.density = 1.f;
		...
		{
			b2EdgeShape es;
			es.Set(SfToBoxVec(shape[0].position), SfToBoxVec(shape[1].position));
			f.shape = &es;
		...
		else
		{
			...
			for (auto i = 0u; i < pointCount; i++)
				vertices[i] = SfToBoxVec(shape[i].position);
	...
	//{
	//	bodyDef.position.Set(0.f, 0.f);
	//	bodyDef.position = HelperFunctions::SfToBoxVec(object.getPosition());
	//	body = world.CreateBody(&bodyDef);

	//	const sf::VertexArray& points = object.getPolyPoints();
...
void BodyCreator::createFixture(const sf::VertexArray& points, int s, b2Body* body)
{
	bool clockwise = (getWinding(points[0].position, points[1].position, points[2].position) > 0.f);
	std::unique_ptr<b2Vec2[]> vertices(new b2Vec2[s]);
	for (auto i = 0u; i < s; i++)
	{
		//reverse direction if verts wound clockwise
		int index = (clockwise) ? (s - 1) - i : i;
		vertices[i] = SfToBoxVec(points[index].position);
	}

	b2PolygonShape ps;
	ps.Set(vertices.get(), s);
	b2FixtureDef f;
	f.density = 1.f;
	f.shape = &ps;
